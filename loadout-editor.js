...
const cropCheckBox = documen.getElementById('crop-image');

function updatePreviewFit() {
	const previewImg = document.getElementById('image-preview-img');
	if (previewImg.style.display !== 'none') {
		previewImv.style.objectFit = cropCheckBox.checked ? 'cover' : 'fill';		
	}
}
...
document.getElementById('save-btn').addEventListener('click'), async () => {
	...
	if (result && result.success) {
		window.location.href = `profile.html?character=${characerName}&loadout=${name}`;
	} else {
